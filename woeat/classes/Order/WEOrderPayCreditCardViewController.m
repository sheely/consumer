//
//  WEOrderPayCreditCardViewController.m
//  woeat
//
...
@interface WEOrderPayCreditCardViewController ()
{
...
@implementation WEOrderPayCreditCardViewController
...
- (void)viewDidLoad {
    ...
    [orderList setUpLeftText:@[@"订单金额", @"优惠券", @"应付金额"] rightText:@[totalValue, couponValue, payValue]];
...
//    WECreditCardInputView *creditCardView = [[WECreditCardInputView alloc] initWithRecharge:NO];
...
//    _creditCardView = creditCardView;
    WESelectBankOrInputView *selectCardView = [[WESelectBankOrInputView alloc] initWithRecharge:NO];
    ...
    [confirmPayButton setTitle:@"确认付款" forState:UIControlStateNormal];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [UmengStat pageEnter:UMENG_STAT_PAGE_PAY_CARD];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [UmengStat pageLeave:UMENG_STAT_PAGE_PAY_CARD];
}
...
- (void)confirmPayButtonTapped:(UIButton *)button
{
    ...
    if ([_selectCardView isSelectSaveCard]) {
        if (cardId.length) {
            [WENetUtil CheckoutUsingSavedBankCardWithBankCardId:cardId
                                                        OrderId:_order.Id
        ...
        [WENetUtil CheckoutUsingBankCardWithOrderId:_order.Id
                                           CardType:@""
